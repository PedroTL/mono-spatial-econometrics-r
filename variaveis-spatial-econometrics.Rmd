...
gini <- read_xlsx("C:\\Users\\pedro\\Documents\\GitHub\\mono-spatial-econometrics-r\\Mono - Bancos de Dados\\gini.xlsx") %>%
  ...
  select(gini_income_code_muni = code_muni, gini_2010 = `2010`)
```
...
```{r, warning=FALSE, message=FALSE}
income_percapta <- read_xlsx("C:\\Users\\pedro\\Documents\\GitHub\\mono-spatial-econometrics-r\\Mono - Bancos de Dados\\renda_percapta_2010tabnet.xlsx") %>%
  rename(income_percapta = Renda_média_domic._per_capita) %>%
  mutate(code_muni = gsub("[^0-9.-]", "", Município),
         code_muni = as.factor(code_muni)) %>%
  select(gini_income_code_muni = code_muni ,income_percapta)
```
...
```{r, warning=FALSE, message=FALSE}
density <- read_xlsx("C:\\Users\\pedro\\Documents\\GitHub\\mono-spatial-econometrics-r\\Mono - Bancos de Dados\\dens_demogr_censo_2010completa.xlsx") %>%
...
final_df_model <- join_all(list(man_predispositiondf,
                                ...
                                density,
                                education,
                                no_religion), by = "code_muni", type = "left")
...
write.xlsx(final_df_model, "C:\\Users\\pedro\\Documents\\GitHub\\mono-spatial-econometrics-r\\Mono - Bancos de Dados\\final_df_model.xlsx")
```
